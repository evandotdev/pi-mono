...
type ExtractionDialogResult =
	| { kind: "ok"; result: ExtractionResult | null }
...
/**
 * Use the current OpenAI Codex model when possible, otherwise prefer GPT-5.4 mini,
 * then fall back to any available OpenAI Codex model.
 */
function selectExtractionModel(
	...
	const preferredModel = modelRegistry.find("openai-codex", CODEX_MODEL_ID);
	if (preferredModel) {
...
function normalizeExtractionResult(value: unknown): ExtractionResult | null {
	...
	const questionsValue = (value as { questions?: unknown }).questions;
	...
	for (const item of questionsValue) {
		...
		questions.push(context === undefined ? { question } : { question, context });
	}
...
export default function (pi: ExtensionAPI) {
	...
	const answerHandler = async (ctx: ExtensionContext) => {
			...
			if (!extractionModel) {
				ctx.ui.notify("No openai-codex model available for question extraction", "error");
				return;
			}

			const extractionAuth = await ctx.modelRegistry.getApiKeyAndHeaders(extractionModel);
			...
			const extractionResult = await ctx.ui.custom<ExtractionDialogResult>((tui, theme, _kb, done) => {
				const loader = new BorderedLoader(tui, theme, `Extracting questions using ${extractionModel.id}...`);
				loader.onAbort = () => done({ kind: "ok", result: null });

				const doExtract = async (): Promise<ExtractionDialogResult> => {
					...
					const response = await complete(
						...
						{ apiKey: extractionAuth.apiKey, headers: extractionAuth.headers, signal: loader.signal },
					);
					...
					if (!parsed) {
						return { kind: "error", error: "Failed to parse extracted questions as JSON" };
					}
